...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    fetchProducts({ sort: 'newest' }).then((items) => setProducts(items.slice(0, 6)))
  }, [])

  return (
    <>
      <section className="relative pt-20 pb-24 overflow-hidden bg-white">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-brand-50/50 via-transparent to-transparent"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="text-left">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand-50 text-brand-600 rounded-full text-xs font-bold mb-6">
                <span className="flex h-2 w-2 rounded-full bg-brand-500 animate-pulse"></span>
                Over 500+ new items listed today
              </div>
              <h1 className="text-5xl md:text-6xl font-extrabold text-gray-900 mb-6 leading-[1.1] tracking-tight">
                Everything for your <br />
                <span className="text-brand-500">Premium Kitchen.</span>
              </h1>
              ...
              <div className="flex flex-wrap gap-4">
                <Link to="/products" className="px-8 py-4 bg-brand-500 text-white font-bold rounded-2xl hover:bg-brand-600 shadow-xl shadow-brand-500/20 transition-all active:scale-95">
                  Explore Kitchen
                </Link>
                <Link to="/contact" className="px-8 py-4 bg-white text-gray-900 border border-gray-200 font-bold rounded-2xl hover:bg-gray-50 transition-all active:scale-95">
                  Our Story
                </Link>
              </div>
            </div>
      ...
      {!supabaseReady ? (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Notice>
            Supabase is not connected yet. Add your keys in <code>.env</code>; meanwhile the app shows sample catalog data.
          </Notice>
        </div>
      ) : null}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-20">
        <div className="bg-gray-900 rounded-[2.5rem] p-12 relative overflow-hidden">
          <div className="absolute inset-0 bg-brand-500 opacity-10 blur-3xl -right-1/4 -top-1/4"></div>
          <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Link to="/bulk-order" className="px-8 py-4 bg-brand-500 text-white font-bold rounded-2xl hover:bg-brand-600 transition-all shadow-xl shadow-brand-500/20">
                Get Bulk Quote
              </Link>
